    ...
    try:
        if os.path.exists(DATA_FILE):
            with open(DATA_FILE, "r") as f:
                ...
                return data.get("portfolios", {}), datetime.fromisoformat(data.get("current_date", datetime.now().isoformat())), data.get("price_history", {}), data.get("portfolio_history", {})
    except:
        pass
    return {}, datetime.now(), {}, {}
...
tab1, tab3, tab5 = st.tabs(["📊 Market", "💼 Portfolio", "🚀 Simulate"])
...
with tab3:
    ...
    if player in portfolios:
        ...
        if st.button(action, type="primary"):
            ...
            st.info(f"**Current market price of {tr_ticker}**: {current_price:,.2f} GC")
