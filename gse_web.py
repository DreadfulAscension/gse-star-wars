...
<style>
    ...
    .report-card { background-color: #0a1428; border: 1px solid #336699; padding: 20px; border-radius: 8px; margin: 15px 0; }
</style>
    ...
    try:
        if os.path.exists(DATA_FILE):
            with open(DATA_FILE, "r") as f:
                ...
                return (data.get("portfolios", {}), datetime.fromisoformat(data.get("current_date", datetime.now().isoformat())), 
                        data.get("price_history", {}), data.get("portfolio_history", {}))
    except: pass
    return {}, datetime.now(), {}, {}

def save_data(portfolios, current_date, price_history, portfolio_history):
    try:
        data = {"stocks": {t: stocks[t]["price"] for t in stocks},
                "portfolios": portfolios, "current_date": current_date.isoformat(),
                "price_history": price_history, "portfolio_history": portfolio_history}
...
def simulate_week():
    # Your original simulate_week function (unchanged)
...
# ====================== TABS ======================
tabs = st.tabs(["📊 Market", "📈 Charts", "💼 Portfolio", "📋 Detailed Report", "📈 Performance", "🚀 Simulate", "🏦 Takeover"])
    ...
    if player in portfolios:
        ...
        if st.button(action, type="primary"):
            ...
            if action == "Buy":
                ...
                if tr_ticker in p["holdings"] and p["holdings"][tr_ticker] >= qty:
...
with tabs[3]:  # Detailed Report
    ...
    if report_player in portfolios:
        ...
        if report:
            ...
            with col1:
                if report["holdings"]:
                    fig = px.pie(pd.DataFrame(report["holdings"]), names="Ticker", values="Market Value", 
            ...
            with col2:
                if report["holdings"]:
                    ...
                    fig2 = px.bar(df_hold, x="Ticker", y="Unrealized P&L", 
                                 title="Unrealized P&L per Stock", color="Unrealized P&L")
            ...
            # Printable / Export
            if st.button("🖨️ Generate Printable Report"):
                st.markdown("### Full GTN Intelligence Report", unsafe_allow_html=True)
                st.json(report)
                st.info("Use browser print (Ctrl+P) → Save as PDF for official record.")
    else:
        st.warning("Please create a portfolio first.")

# Other tabs (Market, Charts, Performance, Simulate...) - use the same structure as previous version
